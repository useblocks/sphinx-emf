...
class SphinxEmfCommonConfig(BaseModel):
    ...
    """
    Function that shall be called before reading the M1 model.

    Input parameter is the M2 ``ResourceSet``.
    Must return the M2  ``ResourceSet`` again after modifying it.

    This can be used to add custom model parts that are not part of the M2 model.
    """

    emf_post_read_hook: Optional[Callable[[XMIResource], List[Any]]] = None
    """
    Function that shall be called after reading the M1 model.

    Input paramter is the M1 ``XMIResource``.
